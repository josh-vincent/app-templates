...
fleshed-out example app (FitStake — virtual-money fitness wagers backed by
...
It rewrites every occurrence of `FitStake` / `fitstake` / `FITSTAKE` /
`com.tocld.fitstake` across `.ts`, `.tsx`, `.json`, `.md`, etc. It does
...
`fitstake` (i.e. it's already been renamed). If you forked the template
...
| RevenueCat IAP | `convex/iapHttp.ts`, `convex/iapSkus.ts`, `components/RevenueCatProvider.tsx`, `app/hooks/useFitStakeProGate.ts`, `react-native-purchases*` from `package.json`, `EXPO_PUBLIC_REVENUECAT_*` from `.env.example` |
...
MIT. The bundled FitStake example assets (icons, mascots) ship under the

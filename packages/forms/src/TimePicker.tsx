/**
 * TimePicker — themed time picker with animated/classic/underlined variants.
 *
 * @package    @jv/forms
 * @since      0.1.0
 * @stability  stable
 ...
 * @demo       ./TimePicker.demo.tsx
 * @donor      fitstake/components/forms/TimePicker.tsx
 */
...
export const TimePicker: React.FC<TimePickerProps> = ({
  ...
  const picker =
    Platform.OS === 'ios' ? (
      ...
      visible && (
        <DateTimePicker value={value || new Date()} mode="time" is24Hour={is24Hour} display="default" onChange={handleChange} />
      )
    );
      ...
      <View className={`mb-global ${containerClassName}`}>
        ...
        <View className="relative">
          <TouchableOpacity
            onPress={show}
            disabled={disabled}
            className={`border rounded-lg py-4 px-3 h-14 pr-10 bg-transparent ${isFocused ? 'border-black dark:border-white' : 'border-black/60 dark:border-white/60'} ${error ? 'border-red-500' : ''} ${disabled ? 'opacity-50' : ''}`}
          >
      ...
      <View className={`mb-global ${containerClassName}`}>
        <View className="relative">
          ...
          <TouchableOpacity
            onPress={show}
            disabled={disabled}
            className={`border-b-2 py-4 px-0 h-14 pr-10 bg-transparent border-t-0 border-l-0 border-r-0 ${isFocused ? 'border-black dark:border-white' : 'border-black/60 dark:border-white/60'} ${error ? 'border-red-500' : ''} ${disabled ? 'opacity-50' : ''}`}
          >
            <ThemedText className={value ? 'text-base' : 'text-base text-gray-500'}>
              {value ? formatTime(value, is24Hour) : ''}
            </ThemedText>
          </TouchableOpacity>

...
fleshed-out example app (Aurora — virtual-money fitness wagers backed by
...
It rewrites every occurrence of `Aurora` / `aurora` / `AURORA` /
`com.tocld.aurora` across `.ts`, `.tsx`, `.json`, `.md`, etc. It does
...
`aurora` (i.e. it's already been renamed). If you forked the template
...
| RevenueCat IAP | `convex/iapHttp.ts`, `convex/iapSkus.ts`, `components/RevenueCatProvider.tsx`, `app/hooks/useAuroraProGate.ts`, `react-native-purchases*` from `package.json`, `EXPO_PUBLIC_REVENUECAT_*` from `.env.example` |
...
MIT. The bundled Aurora example assets (icons, mascots) ship under the

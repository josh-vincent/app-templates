...
fleshed-out example app (HealthPulse — virtual-money fitness wagers backed by
...
It rewrites every occurrence of `HealthPulse` / `healthpulse` / `HEALTHPULSE` /
`com.tocld.healthpulse` across `.ts`, `.tsx`, `.json`, `.md`, etc. It does
...
`healthpulse` (i.e. it's already been renamed). If you forked the template
...
| RevenueCat IAP | `convex/iapHttp.ts`, `convex/iapSkus.ts`, `components/RevenueCatProvider.tsx`, `app/hooks/useHealthPulseProGate.ts`, `react-native-purchases*` from `package.json`, `EXPO_PUBLIC_REVENUECAT_*` from `.env.example` |
...
MIT. The bundled HealthPulse example assets (icons, mascots) ship under the

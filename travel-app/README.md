...
fleshed-out example app (Voyager — virtual-money fitness wagers backed by
...
It rewrites every occurrence of `Voyager` / `voyager` / `VOYAGER` /
`com.tocld.voyager` across `.ts`, `.tsx`, `.json`, `.md`, etc. It does
...
`voyager` (i.e. it's already been renamed). If you forked the template
...
| RevenueCat IAP | `convex/iapHttp.ts`, `convex/iapSkus.ts`, `components/RevenueCatProvider.tsx`, `app/hooks/useVoyagerProGate.ts`, `react-native-purchases*` from `package.json`, `EXPO_PUBLIC_REVENUECAT_*` from `.env.example` |
...
MIT. The bundled Voyager example assets (icons, mascots) ship under the

...
function resolveFitstakePath(path: string): string | undefined {
  try {
    ...
    // Widget / Live Activity deep links. SwiftUI's widgetURL emits paths
    // without the route-group prefix, so map them onto the tab routes.
    if (stripped === '' || stripped === 'today' || stripped === 'home') {
      return '/(tabs)/(home)';
    }
    const challengeMatch = stripped.match(/^challenges\/([^/?#]+)(?:[/?#].*)?$/i);
    if (challengeMatch) {
      return `/(tabs)/challenges/${encodeURIComponent(challengeMatch[1])}`;
    }
    if (/^challenges\/?$/i.test(stripped)) return '/(tabs)/challenges';
    if (/^wallet\/?$/i.test(stripped)) return '/(tabs)/wallet';
    if (/^jackpot\/?$/i.test(stripped)) return '/(tabs)/jackpot';
    if (/^profile\/?$/i.test(stripped)) return '/(tabs)/profile';
  } catch {
    return undefined;
  }
  return undefined;

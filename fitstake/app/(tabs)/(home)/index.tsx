...
function FriendsPoolHero({
  ...
  const empty = total === 0;
  const hasShare = amIEligible && mySharePct != null && total > 0;
  const stateChip = overtime ? 'PAYING OUT' : settling ? 'CLOSING' : null;
  // Single-eyebrow header: label + countdown collapsed. Status chip appears
  // only at the boundary moments where it actually matters.
  return (
    <Pressable
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        ...
        <Text style={{ ...EYEBROW, color: IRON }}>
          FRIENDS POOL · {fmtEndsIn(settlesAt, nowMs).toUpperCase()} LEFT
        </Text>
        {stateChip ? (
          <View
            style={{
      ...
      <View
        ...
        <Text style={{ color: IRON, opacity: 0.78, fontSize: 13, fontWeight: '600', flexShrink: 1 }}>
          {empty
            ? 'Bet a friend to start it'
            : `${playersCount} player${playersCount === 1 ? '' : 's'} · ${contributorCount} feeding it`}
        </Text>
        {hasShare ? (
          <Text
            style={{
            ...
            {fmtPct(mySharePct)} yours · ~{fmtMoney(myShare!)} now
          </Text>
        ) : (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
            <Text style={{ color: IRON, fontSize: 13, fontWeight: '800' }}>
              {empty ? 'Start' : 'See pool'}
            </Text>
            <Icon name="ChevronRight" size={14} color={IRON} />
          </View>
        )}
      </View>
    </Pressable>
...
export default function TodayScreen() {
  ...
  // Compact pace fact, sits right under the $X. The dollar is the headline;
  // this line is the condition attached to it. No coach prose.
  const headlineSub = mostUrgent
    ? `${fmtSteps(mostUrgent.needed)} short`
    : stepStakes.length > 0
      ? 'on pace'
      : 'start one to play';

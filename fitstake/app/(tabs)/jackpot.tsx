...
function ScopeCard({
  ...
  return (
    <Pressable
      ...
      {/* Header row: scope label + bell. No card chrome — the spectacle
          lives on the iron background per design law. */}
      <View
        style={{
          flexDirection: 'row',
          alignItems: 'center',
      ...
      <Text
        style={{
          marginTop: 4,
          color: colors.text,
          opacity: allLocked ? 0.4 : 0.6,
          fontSize: 13,
        }}>
        {subline}
      </Text>

      {/* Tier unlock stepper — three nodes connected by a hairline. Shows
          the ladder, not a tappable selector. */}
      <View
        style={{
          marginTop: 14,
          flexDirection: 'row',
        ...
        {TIER_ORDER.map((t, idx) => {
          ...
          const nodeColor = ok ? GOLD : colors.text + '30';
          return (
              ...
              {!isLast ? (
                <View
                  style={{
                    ...
                    marginTop: -16,
                    backgroundColor:
                      ok && eligibility[TIER_ORDER[idx + 1]]
                        ? GOLD
                        : colors.text + '20',
                  }}
                />
  ...
  return (
    <View
      ...
      <Text
        style={{
          marginTop: 6,
          color: allUnlocked ? GOLD : LIME,
          fontSize: 12,
          fontWeight: '700',
        }}>
        {allUnlocked
          ? `${settledCount} wins · every tier in reach.`
          : lockedNext
            ? `${settledCount} of ${TIER_THRESHOLDS[lockedNext]} wins · ${TIER_THRESHOLDS[lockedNext] - settledCount} to go for ${TIER_LABELS[lockedNext].toLowerCase()}.`
            : ''}
      </Text>
    </View>
  );
